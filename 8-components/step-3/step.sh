...
for instance in "${!workerExternalIps[@]}"; do
  scp -i ~/.ssh/formation ${instance}.kubeconfig formation@${workerExternalIps[$instance]}:~/
  scp -i ~/.ssh/formation kube-proxy.kubeconfig formation@${workerExternalIps[$instance]}:~/
done
